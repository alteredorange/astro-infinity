...
export async function get(params, request) {
  ...
  const CHwidth =
    ...
    Math.round(request.headers.get("width") * (1 / CHdpr));
  console.log({ CHwidth });
  ...
  const originalFormat = src
    .substring(src.lastIndexOf(".") + 1)
    .toLocaleLowerCase(); //gets the original format

  console.log({ width });
  const isAnimated =
    animated == "true" || originalFormat == "gif" ? true : false; //sets if animated
